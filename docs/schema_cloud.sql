-- Funnel ClickHouse Schema (Cloud Edition)
-- Version: 2.0
-- Description: Schema optimized for ClickHouse Cloud (SharedMergeTree)
--
-- This schema is designed for ClickHouse Cloud which uses SharedMergeTree.
-- It does NOT include projections (use schema_self_hosted.sql for self-hosted
-- deployments that can take advantage of projections).
--
-- Usage:
--   Deploy via: ansible-playbook playbooks/schema.yml
--   Or manually: clickhouse-client --multiquery < schema_cloud.sql
--
...
--
-- CHANGELOG (v2.0):
-- - Removed partitions (not needed for data lifecycle, improves query performance)
-- - Fixed SummingMergeTree aggregation in video_stats (correctness bug)
-- - Added materialized columns for video tag extraction
-- - Switched to ARRAY JOIN syntax for cleaner MV definitions
-- - Removed redundant tag_array column from event_tags_flat_data
-- - Separated MV target tables from MV definitions for clarity
